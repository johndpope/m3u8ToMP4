...
//        let string2 = NSBundle.mainBundle().pathForResource("113", ofType: "ts")
        ...
        let compositionTrack1 = composition.addMutableTrack(withMediaType: AVMediaTypeVideo, preferredTrackID: kCMPersistentTrackID_Invalid)
        compositionTrack1.preferredVolume = 1.0
        
        let videoAsset1 = AVURLAsset.init(url: url1, options: nil)
        
        let trackArray1 = videoAsset1.tracks(withMediaType: AVMediaTypeVideo)
        print(trackArray1)
        let track1 = trackArray1[0]//这里没做异常处理，遇到不支持的格式时数组为空，比如换成ts文件
        let timeRange1 = CMTimeRangeMake(kCMTimeZero, videoAsset1.duration)
        do {
            try compositionTrack1.insertTimeRange(timeRange1, of: track1, at: kCMTimeZero)
        ...
        let trackArray2 = videoAsset2.tracks(withMediaType: AVMediaTypeVideo)
        print(trackArray2)
        let track2 = trackArray2[0]
        let timeRange2 = CMTimeRangeMake(kCMTimeZero, videoAsset2.duration)
        do {
            try compositionTrack1.insertTimeRange(timeRange2, of: track2, at: kCMTimeZero)
